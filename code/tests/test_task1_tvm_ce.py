...
def test_composition_target_branch_is_stop_gradient():
    loss_source = inspect.getsource(Task1TVMSCRepair.loss)
    start = loss_source.index('if composition_weight > 0.0:')
    composition_source = loss_source[start:]

    assert 'with torch.no_grad():' in composition_source
    assert 'h2.detach()' in composition_source
...
def test_taskflow_records_reference_audits_and_complete_eval_metadata():
    script = Path(__file__).parents[1] / 'scripts' / 'run_task1_tvm_sc_repair_2k.sh'
    source = script.read_text(encoding='utf-8')
    remote_config = (
        Path(__file__).parents[1]
        / 'configs' / 'task1_tvm_sc_repair_2k.remote_1x5090.env'
    ).read_text(encoding='utf-8')

    assert 'CALIBRATION_AUDIT_NFE=256,512,1024' in remote_config
    for field in (
            "'physical_t_grid'", "'sc_rule'", "'d50'", "'nfe'",
            "'checkpoint_global_step'", "'weights'", "'seed'",
            "'num_samples'"):
        assert field in source
    assert "'resolution_audit_completed'] = True" in source
    assert 'TRAINING_ROOT_OVERRIDE' in source
    assert 'SC_REPAIR_INITIAL_RESUME_CKPT' in source
    assert 'SC_REPAIR_SKIP_INITIAL_TRAINING' in source
    assert 'SC_REPAIR_SKIP_STEP500_EVAL' in source
...
def test_joint_j0_contract_and_local_time_mapping():
    ...
    assert config.tvm_sc_comp_enabled is False
    assert config.tvm_sc_comp_weight == 0.0
    ...
    assert Task1TVMJointJ0._sc_loss_weights(owner) == (0.03, 0.0)
    ...
    script = root / 'scripts/run_task1_tvm_joint_j0.sh'
    subprocess.run(['bash', '-n', str(script)], check=True)


def test_joint_j1_contract_and_target_ema_update():
    root = Path(__file__).parents[1]
    config = OmegaConf.load(root / 'configs/algo/task1_tvm_joint_j1.yaml')
    assert config.name == 'task1_tvm_joint_j1'
    assert config.tvm_local_weight == 0.15
    assert config.tvm_local_warmup_steps == 0
    assert config.tvm_training_target_ema_decay == 0.99
    assert config.tvm_sc_comp_enabled is False
    assert config.tvm_sc_comp_weight == 0.0
    assert config.tvm_endpoint_weight == 0.0
    ...
    owner = types.SimpleNamespace(
        backbone=online,
        training_target_ema=target,
        training_target_ema_decay=0.99,
        training_target_ema_updates=torch.zeros((), dtype=torch.long),
        local_weight=0.15,
        local_warmup_steps=0,
    )
    Task1TVMJointJ1._update_training_target_ema(owner)
    torch.testing.assert_close(
        target.weight, torch.full_like(target.weight, 0.01))
    assert int(owner.training_target_ema_updates) == 1
    assert Task1TVMJointJ1._local_lambda(owner) == 0.15
    assert Task1TVMJointJ1._local_warmup_is_valid(owner)

    query_source = inspect.getsource(Task1TVMJointJ1._training_target_local)
    assert 'torch.zeros_like(t' in query_source
    assert 'self.training_target_ema(' in query_source
    assert 'x_self_cond=h' in query_source
    assert '_map_teacher_local' in inspect.getsource(Task1TVMSCRepair.loss)
    assert '_map_teacher_local' in inspect.getsource(
        Task1TVMSCRepair._run_gradient_conflict_probe)

    script = root / 'scripts/run_task1_tvm_joint_j1.sh'
    subprocess.run(['bash', '-n', str(script)], check=True)


def test_endpoint500_contract_locks_online_bank_rolling_teacher_and_zero_sc_data():
    root = Path(__file__).parents[1]
    config = OmegaConf.load(root / 'configs/algo/task1_tvm_endpoint500.yaml')
    assert config.name == 'task1_tvm_endpoint500'
    assert list(config.tvm_endpoint_grid_physical) == [
        0.0, 0.3392771, 0.5814685, 0.7246688, 1.0]
    assert config.endpoint_reference_sc_mode == 'interval_zero_then_rolling'
    assert config.student_sc_mode == 'closed_no_sc'
    assert config.tvm_local_time_sampling == 'v1_m_tau25_global256'
    assert config.tvm_local_sc_probability == 0.0
    assert config.tvm_endpoint_refresh_step == 250
    assert config.tvm_endpoint_reference_steps == 128
    assert config.tvm_endpoint_target_gradient_ratio == 0.25
    assert config.tvm_data_target_gradient_ratio == 0.125

    reference_source = inspect.getsource(
        Task1TVMEndpoint500._teacher_finite_target)
    assert 'h = None' in reference_source
    assert 'h = self.teacher_model.vocab_embed(probability).detach()' in reference_source
    bank_source = inspect.getsource(Task1TVMEndpoint500.build_endpoint_bank)
    assert '_online_endpoint_probability' in bank_source
    refresh_source = inspect.getsource(
        Task1TVMEndpoint500.on_train_batch_start)
    assert 'build_endpoint_bank(self.endpoint_refresh_step)' in refresh_source
    local_source = inspect.getsource(Task1TVMEndpoint500._local_data_branch)
    assert 'torch.zeros_like(t)' in local_source
    assert 'x_self_cond=None' in local_source

    owner = types.SimpleNamespace(
        device=torch.device('cpu'),
        config=types.SimpleNamespace(seed=7),
        global_step=11,
        _task1_lut_anchor_physical_t=[0.0, 1.0 / 128.0],
        _t_to_tau=lambda value: value,
        _task1_physical_time=lambda value: value,
    ...
    groups = []
    for accumulation_step in range(8):
        _, group = Task1TVMEndpoint500._sample_local_times(
            owner, 16, accumulation_step)
        groups.append(group)
    combined = torch.cat(groups)
    assert tuple(int((combined == index).sum()) for index in range(7)) == (
        3, 3, 12, 21, 46, 34, 9)
...
if __name__ == '__main__':
    ...
    print(f'{len(tests)} Task1 TVM/SC-repair tests passed')
