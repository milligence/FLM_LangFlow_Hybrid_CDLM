...
from experiment_callbacks import (
...
class CosineDecayWarmupLRScheduler(
    ...
    def step(self, epoch=None):
        ...
        constant_lr = getattr(self, '_task1_constant_lr', None)
        if constant_lr is not None:
            for group in self.optimizer.param_groups:
                group['lr'] = float(constant_lr)
            self._last_lr = [
                float(constant_lr) for _ in self.optimizer.param_groups]
            return
