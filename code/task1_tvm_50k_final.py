...
def _f_map_terms(logits, tangent, b_raw, b_tangent, eta):
    eta = eta[:, None, None]
...
def _f_map_loss(value, value_tangent, eta, teacher):
    eta = eta[:, None, None]
    residual = (
        (1.0 - eta) * value
...
def _f_map_tokens(logits, tangent, b_raw, b_tangent, eta, teacher):
    value, value_tangent = _f_map_terms(
        logits, tangent, b_raw, b_tangent, eta)
    return _f_map_loss(value, value_tangent, eta, teacher), value
...
class Task1TVM50KFinal(LangFlowFLMHybrid):
    ...
    def __init__(self, config, tokenizer):
        ...
        self._compiled_f_map_terms = None
        ...
        self._sampler_profile = 'pre32'
        self._profile_activation_step = 0
        self._profile_recalibration_step = None
    ...
    def _map_rows(self, step, generator):
        ...
        profile = None
        if self.line == 'F' and 'profiles' in map_config:
            profile_name = self._sampler_profile
            if step < 32000:
                profile_name = 'pre32'
            profile = map_config['profiles'][profile_name]
        ...
        for label, raw_count in counts.items():
            ...
            kind = spec['kind']
            if kind == 'deployment':
                all_grids = map_config['deployment']['grids']
                grid_names = spec.get('grids', list(all_grids))
                grids = {name: all_grids[name] for name in grid_names}
                indices = spec['interval_indices']
                interval_rows = None
                row_generator = generator
                if label == 'D_patch' and profile is not None:
                    interval_rows = [
                        int(value) for value in profile['D_patch_interval_rows']]
                    if len(interval_rows) != len(indices) or sum(interval_rows) != count:
                        raise ValueError('D_patch interval allocation does not match its quota.')
                    if any(value % 2 for value in interval_rows):
                        raise ValueError('D_patch exact/jitter allocation must be even per interval.')
                    row_generator = self._generator('map_pair_D_patch', step)
                else:
                ...
                for grid_name, grid_values in grids.items():
                    for interval_offset, interval in enumerate(indices):
                        per_interval = (
                            interval_rows[interval_offset]
                            if interval_rows is not None else per)
                        for sample_index in range(per_interval):
                            ...
                            if jittered:
                                ...
                                for node in range(1, len(nodes) - 1):
                                    nodes[node] += self._draw(
                                        row_generator, float(low), float(high),
                                        'uniform')
            ...
            for _ in range(count):
                ...
                rows.append(self._row(
                    label, r, s, None, None, False, step,
                    source='analytical_data'))
    ...
    def _row(self, label, r, s, interval, grid, jittered, step, *,
             nodes=None, source='analytical_data'):
        if not 0.0 <= r < s <= 0.9500001:
            raise ValueError(f'Infeasible map pair r={r}, s={s}.')
        hard = label in {'L', 'H'} or (
            label in {'D', 'D_patch'} and interval == 3)
    ...
    def _finite_jvp_call(
            self, embedding, embedding_jvp, sigma, sigma_jvp,
            features, feature_jvp):
        if self._compiled_finite_jvp is None:
            if self.line == 'F':
                def target(x, x_jvp, time, time_jvp, time_features,
                           time_features_jvp):
                    ((residual, output_features),
                     (tangent, output_features_jvp)) = (
                        self.backbone.forward_with_jvp(
                        x, x_jvp, time, sigma_jvp=time_jvp,
                        inputs_are_embeddings=True,
                        finite_time_features=time_features,
                        finite_time_features_jvp=time_features_jvp,
                        return_output_features=True))
                    head = self.backbone.finite_correction_head
                    return (
                        residual, tangent, head(output_features),
                        F.linear(output_features_jvp, head.weight, None))
            else:
                def target(x, x_jvp, time, time_jvp, time_features,
                           time_features_jvp):
                    return self.backbone.forward_with_jvp(
                        x, x_jvp, time, sigma_jvp=time_jvp,
                        inputs_are_embeddings=True,
                        finite_time_features=time_features,
                        finite_time_features_jvp=time_features_jvp)
            if bool(getattr(
                ...
                # Lightning wraps this single-GPU job in DDP.  PyTorch 2.8's
                # DDP graph partitioner mishandles symbolic scalar temporaries
                # in this dynamic layer-JVP graph; no DDP partitioning benefit
                # exists at world size one.
        ...
        return self._compiled_finite_jvp(
            embedding, embedding_jvp, sigma, sigma_jvp,
            features, feature_jvp)
    ...
    def finite_with_eta_jvp(self, state, r, eta):
        ...
        self.backbone.eval()
        try:
            processed_condition = self._process_sigma(condition)
            if self.line == 'F':
                residual, tangent, b_raw, db_raw = self._finite_jvp_call(
                    embedding, zeros, processed_condition,
                    torch.zeros_like(processed_condition), features,
                    feature_tangent)
            else:
        ...
        if self.line == 'F':
            output['b_raw'] = b_raw.float()
            output['db_raw'] = db_raw.float()
        return output

    def _f_objective_terms(self, logits, tangent, b_raw, b_tangent, eta):
        if self._compiled_f_map_terms is None:
            target = _f_map_terms
            if bool(getattr(
            ...
            self._compiled_f_map_terms = target
        return self._compiled_f_map_terms(
            logits, tangent, b_raw, b_tangent, eta)
    ...
    @torch.no_grad()
    def _detached_prefix_rollout_source(self, initial_noise, row):
        if row.get('class') != 'D_patch':
            raise RuntimeError(
                'Detached rollout source is restricted to the eight D_patch rows.')
        nodes = row.get('nodes')
        interval = int(row.get('interval', -1))
        if nodes is None or interval < 0 or len(nodes) != 5:
            raise RuntimeError('D_patch rollout source requires its four-step grid.')
        state = initial_noise.detach().float()
        previous_mode = self.backbone.training
        self.backbone.eval()
        try:
            for left, right in zip(nodes[:interval], nodes[1:interval + 1]):
                r = torch.full(
                    (state.shape[0],), float(left), device=self.device)
                s = torch.full_like(r, float(right))
                eta = (s - r) / (1.0 - r)
                value = self._finite_generation_query(state, r, eta)
                state = (
    ...
    def _map_objective(
        ...
        for begin in range(0, len(rows), self.map_microbatch):
            ...
            rollout_indices = [
                ...
                if row.get('source') == 'detached_current_online_cold_prefix']
            if rollout_indices:
                rollout_states = []
                for index in rollout_indices:
                    rollout_states.append(self._detached_prefix_rollout_source(
                        noise[index:index + 1], chunk_rows[index]))
                state = state.clone()
                state[torch.tensor(
                    rollout_indices, device=self.device, dtype=torch.long)] = (
                        torch.cat(rollout_states, dim=0))
            output = self.finite_with_eta_jvp(state, r, eta)
            if self.line == 'F':
                endpoint_value, endpoint_tangent = self._f_objective_terms(
            ...
            if self.line == 'F':
                tokens_loss = _f_map_loss(
                    endpoint_value, endpoint_tangent, eta, target.prob)
            else:
    ...
    def _calibration_due(self, step):
        ...
        if step < first or step == int(self.last_calibration_step):
            return False
        if self._profile_recalibration_step == step:
            return True
        if step <= 10000:
    ...
    @torch.no_grad()
    def _record_f2k_optimizer_update(self, optimizer, completed):
        if (self.line != 'F' or completed < 32000
                or completed % 2000 != 0):
            return
        base_optimizer = getattr(optimizer, 'optimizer', optimizer)
        backbone_ids = {id(parameter) for parameter in self.backbone.parameters()}
        update_square = torch.zeros((), device=self.device, dtype=torch.float64)
        before_square = torch.zeros((), device=self.device, dtype=torch.float64)
        parameter_count = 0
        group_lrs = {}
        group_update_squares = {}
        ...
        for group in base_optimizer.param_groups:
            ...
            if weight_decay != 0.0:
                raise RuntimeError(
                    'F 2k update diagnostic requires the contracted zero weight decay.')
            group_name = str(group.get('name', 'unnamed'))
            group_lrs[group_name] = learning_rate
            group_update_squares.setdefault(
                group_name,
                torch.zeros((), device=self.device, dtype=torch.float64))
            group_parameter_counts.setdefault(group_name, 0)
            for parameter in group['params']:
                ...
                if not state or 'exp_avg' not in state or 'exp_avg_sq' not in state:
                    continue
                optimizer_step = state.get('step', completed)
                if isinstance(optimizer_step, torch.Tensor):
                    optimizer_step = float(optimizer_step.detach().cpu())
                else:
                ...
                denominator = (
                    exp_avg_sq.sqrt() / math.sqrt(bias_correction2) + epsilon)
                delta = -(
                    learning_rate / bias_correction1) * exp_avg / denominator
                before = parameter.detach().float() - delta
                update_square.add_(delta.double().square().sum())
                before_square.add_(before.double().square().sum())
                parameter_count += parameter.numel()
                group_update_squares[group_name].add_(
                    delta.double().square().sum())
                group_parameter_counts[group_name] += parameter.numel()
        update_norm = float(update_square.sqrt())
        parameter_norm_before = float(before_square.sqrt())
        denominator = max(
            parameter_norm_before,
            1.0e-6 * math.sqrt(max(parameter_count, 1)))
        event = {
            ...
            'update_weight_ratio': update_norm / denominator,
            'group_learning_rates': group_lrs,
            'group_update_norms': {
                name: float(value.sqrt())
                for name, value in group_update_squares.items()},
            'group_update_rms': {
                name: float((group_update_squares[name]
                            / max(group_parameter_counts[name], 1)).sqrt())
                for name in group_update_squares},
            'definition': 'exact AdamW delta reconstructed from post-step moments; weight_decay=0',
        }
        if self.trainer.is_global_zero:
            write_controller_event(
                Path(self.config.checkpointing.save_dir)
                / 'f2k_optimizer_update_events.jsonl', event)
    ...
    def optimizer_step(self, *args, **kwargs):
        ...
        self._record_f2k_optimizer_update(optimizer, completed)
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint['task1_tvm_final'] = {
            ...
            'sampler_profile': self._sampler_profile,
            'profile_activation_step': self._profile_activation_step,
            'decision_events': self._decision_events,
            'random_streams': [
                'plan', 'local_noise', 'map_noise', 'map_pair_D_patch',
                'validation'],
        }
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        if record is not None:
            ...
            self._sampler_profile = str(record.get('sampler_profile', 'pre32'))
            self._profile_activation_step = int(
                record.get('profile_activation_step', 0))
            self._decision_events = list(record.get('decision_events', []))
        requested = os.environ.get('TASK1_F_SAMPLER_PROFILE')
        if self.line == 'F' and requested:
            profiles = self.sampler_contract['map'].get('profiles', {})
            if requested not in profiles:
                raise RuntimeError(f'Unknown requested sampler profile {requested!r}.')
            completed = int(checkpoint.get('global_step', 0))
            current = self._sampler_profile
            allowed = {
                32000: {('pre32', 'uniform')},
                36000: {('uniform', 'late_trial'), ('uniform', 'off')},
                40000: {('late_trial', 'late_fixed'),
                        ('late_trial', 'uniform'), ('late_trial', 'off')},
            }
            transition = (current, requested)
            if requested != current and transition not in allowed.get(completed, set()):
                raise RuntimeError(
                    f'Illegal sampler profile transition {transition} at {completed}.')
            if requested != current:
                old_plan = profiles[current]
                new_plan = profiles[requested]
                changed = old_plan != new_plan
                event = {
                    'event_id': f'profile-{completed}-{current}-to-{requested}',
                    'completed_step': completed,
                    'action_first_update': completed + 1,
                    'old_profile': current,
                    'new_profile': requested,
                    'sampling_plan_changed': changed,
                    'model_optimizer_ema_rng_reset': False,
                }
                if event['event_id'] not in {
                        value.get('event_id') for value in self._decision_events}:
                    self._decision_events.append(event)
                self._sampler_profile = requested
                self._profile_activation_step = completed
                if changed:
                    self._profile_recalibration_step = completed
    ...
    def _generate_canonical(self, num_samples, forward_budget):
        ...
        configured_k = int(getattr(
            self.config.algo, 'task1_eval_canonical_k', 0))
        effective_k = (
            configured_k if configured_k > 0
            else max(int(self.canonical_k), int(self.canonical_previous_k)))
        if effective_k not in {1, 2, 4}:
            raise ValueError('Canonical evaluation K must be one of 1, 2, or 4.')
        ...
        for left, right in zip(nodes[:-1], nodes[1:]):
            ...
            if configured_k > 0:
                ...
                for _ in range(effective_k):
                    _, _, probability = self.local_posterior(
                        state, t, cache, model=self.backbone,
                        dropout_enabled=False)
                    cache = detached_self_conditioning_embedding(
                        probability, self.backbone.vocab_embed).detach()
                queries = effective_k
