...
LINE="${1:?Usage: run_30k_audit.sh f CHECKPOINT RUN_ROOT OUTPUT_ROOT}"
CHECKPOINT="${2:?Missing 30k checkpoint path}"
RUN_ROOT="${3:?Missing training run root}"
OUTPUT_ROOT="${4:?Missing audit output root}"
if [[ "$LINE" != f ]]; then
  echo "30k follow-up audit is defined only for F" >&2
  exit 2
fi
if [[ ! -f "$CHECKPOINT" || ! -f "$CHECKPOINT.sha256" ]]; then
  echo "30k checkpoint or SHA sidecar is missing" >&2
  exit 3
fi
...
for MODEL_TYPE in online target_ema eval_ema; do
  OUTPUT="$OUTPUT_ROOT/diagnostics/step_030000__${MODEL_TYPE}.json"
  if [[ ! -f "$OUTPUT" ]]; then
    "$TASK1_DRIVER_PYTHON" "$ROOT/tools/run_20k_audit.py" \
      --bindings "$TASK1_BINDINGS" \
      --contract-dir "$TASK1_CONTRACT_DIR" \
      --audit-config "$ROOT/audit_30k.yaml" \
      --line f --checkpoint "$CHECKPOINT" --step 30000 \
      --model-type "$MODEL_TYPE" --scope full --output "$OUTPUT"
  fi
done

for STEP in 5000 10000 15000 20000 30000; do
  ...
  if [[ -f "$CHECKPOINT_AT_STEP" && -f "$CHECKPOINT_AT_STEP.sha256" && ! -f "$OUTPUT" ]]; then
    "$TASK1_DRIVER_PYTHON" "$ROOT/tools/run_20k_audit.py" \
      ...
      --contract-dir "$TASK1_CONTRACT_DIR" \
      --audit-config "$ROOT/audit_30k.yaml" \
      --line f --checkpoint "$CHECKPOINT_AT_STEP" --step "$STEP" \
...
cp "$ROOT/audit_30k.yaml" "$OUTPUT_ROOT/contracts/audit_30k.yaml"
...
"$TASK1_DRIVER_PYTHON" - "$CHECKPOINT" "$ROOT/train_f.yaml" "$OUTPUT_ROOT/metadata.json" <<'PY'
import json
